function [comp,compression_rate,size_before_comp_ter,size_after_comp_ter] = ternary_huffman(text_file)
...
size_before_comp_ter = numel(raw_input);

decoded = dec2bin(comp);
size_after_comp_ter = numel(decoded);

compression_rate = size_after_comp_ter / size_before_comp_ter;

dict(:, 2) = cellfun(@num2str, dict(:, 2), 'UniformOutput', false);

%%%%%%%%%%%%%%%%% Imprime os resultados num fiheiro "ternary_huffman_results.txt" %%%%%%%%%%%%%%%%%
...
fprintf(fileID, ['%s' '%d\n'], "Tamanho do ficheiro antes da compressão(bits):  ", size_before_comp_ter);
fprintf(fileID, ['%s' '%d\n'], "Tamanho do ficheiro depois da compressao(bits):  ", size_after_comp_ter);
